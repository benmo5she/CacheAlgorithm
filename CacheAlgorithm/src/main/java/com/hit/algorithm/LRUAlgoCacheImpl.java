...
public class LRUAlgoCacheImpl<K,V> extends AbstractAlgoCache<K, V> {
	//Use queue for this implementation as we need quick access to the least recently used node, and queue helps us to achive that as we
	//have cheap access to the head of the queue(poll) and cheaper addition to the queue.
	...
	public V getElement(K key) {
		// since we are accessing a node(if exists) we will need to change its order as it was used right now,so it should go to the back of the queue.
