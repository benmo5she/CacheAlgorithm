...
public class SecondChance<K,V> extends AbstractAlgoCache<K, V>{
	...
	///Create this class to store the bit status of each of our keys in the memory without the need to create separate
	//Container or access to a the disk
	//The ram will be a container which contain KeyValuePair<K,Boolean> elements
	//When O(K) serves as the index, and the P(Boolean) serves as the bit value of said node
	...
	//As second chance algorithm uses FIFO,we should use a queue to store our values(using KeyValuePair as described above)
	...
	public V getElement(K key) {
		...
		{
			//If we found a match, we need to set the node bit to be true as the algorithm dictates
	...
	public V putElement(K key, V value) {
		...
		//Iterate over the queue(polling) to find the first node which has its bit value to be false.
		//While meeting elements with bit value of true, set them to false and put them at the back of the line as the algorithm dictates
		//to provide them with a second chance
		//When met a node with bit value of false, remove it from the queue to make space for the new node.
	...
	public void removeElement(K key) {
		//Iterate over the queue to find a matching node,if found one, remove it and break the loop.
		for(KeyValuePair<K,Boolean> foundKey : ramMemory)
		{
			if(foundKey.getIndex().equals(key))
			{
